...
export default function AdminEditForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // Send userEmail so backend can look up the correct ObjectId
      await updateBooking(
        bookingId,
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid sm:grid-cols-2 gap-x-6 gap-y-4 mb-6">
        {/* User email — editable, pre-filled with current owner's email */}
        <div>
      ...
      <div className="flex gap-3">
        ...
        <a href="/admin/bookings" className="btn-secondary">
          Back to Dashboard
        </a>
      </div>
    </form>
